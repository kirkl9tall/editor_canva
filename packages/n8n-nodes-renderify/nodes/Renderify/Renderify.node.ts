...
export class Renderify implements INodeType {
  description: INodeTypeDescription = {
    ...
    name: "renderify",
    // icon: 'file:renderify.svg',  // add an SVG icon to nodes/Renderify/renderify.svg
    group: ["transform"],
    version: 1,
    subtitle: '={{$parameter["operation"]}}',
    description: "Generate images and manage templates with the Renderify API",
    ...
    properties: [
        ...
        options: [
          {
            name: "Generate Image",
            value: "generateImage",
            description: "Render a template with variables and return the image URL",
          ...
          },
          {
            name: "Get Template",
            value: "getTemplate",
            description: "Fetch metadata for a single template by ID",
      ...
      // ── Shared: Template ID ───────────────────────────────────────────────
      {
        ...
        default: "",
        description: "The ID of the template to use (found in the Renderify dashboard)",
      },
            ...
            values: [
              {
                ...
                type: "string",
                default: "",
                description: "Variable placeholder name (e.g. title, subtitle)",
              },
              {
                displayName: "Value",
                name: "value",
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        if (operation === "generateImage") {
          ...
          for (const v of variablesCollection.variable ?? []) {
          ...
          responseData = await this.helpers.httpRequest({
            ...
            body: { templateId, variables, format: format === "auto" ? undefined : format },
            json: true,
          });
